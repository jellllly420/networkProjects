...
class RequestApi(object):
    ...
    def all_api_request(self):
        ...
        while True:
            ...
            time.sleep(20)
        # 5 . 获取结果
        ret = self.get_result_request(taskid=taskid)
        with open(save_path, 'w+') as f: json.dump(ret, f, ensure_ascii=False, indent=2)
...
if __name__ == '__main__':
    name = argv[1]
    # bname = basename(name)
    # bname = bname[:bname.find('.')]
    os.system("ffmpeg -i {}.mp4 -vn  -c:a mp3 {}.mp3".format(name, name))
    api = RequestApi(appid="e6a77238", secret_key="01128930f08e284c9e3eb061d82c3071", upload_file_path="{}.mp3".format(name))
    api.all_api_request()
